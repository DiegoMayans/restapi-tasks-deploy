...
export const findAllTasks = async (req, res) => {
    try {
        ...
        // En lugar de find() usamos paginate() gracias al modulo mongoose-paginate
        // paginate( documentosAConsultar, opciones )
        ...
        res.status(500).json({
            message: error.message || 'Something went wrong while retrieving the tasks'
        });
    }
};
...
export const findOneTask = async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({
            message: error.message || `Error while retrieving task with id: ${id}`
        });
    }
};
